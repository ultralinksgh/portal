...
if($_SERVER['REQUEST_METHOD']=='POST'){
    if(empty($_POST['_token']) || $_POST['_token'] != $_SESSION['_token']){
        ...
        foreach($courseIds as $courseid){
            $conn = $db->connection();
            $exec = mysqli_query($conn, "INSERT INTO course_registrations(courseid,trimester,academic_year,is_late_registration) VALUES('$courseid', '$trimester','$academicYear', 1)") or die(mysqli_error($conn));
        }
        if($exec){
            echo 'success';
        }
